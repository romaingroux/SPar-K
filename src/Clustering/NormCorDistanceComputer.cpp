...
#include "NormCorDistanceComputer.hpp"
...
NormCorDistanceComputer::NormCorDistanceComputer(size_t shift) throw (std::invalid_argument)
    : CorDistanceComputer::CorDistanceComputer(shift)
{}

NormCorDistanceComputer::~NormCorDistanceComputer()
{}

double NormCorDistanceComputer::compute(const std::vector<double>& x,
...
{   // reallocate buffers if neeeded
    ...
    double min_distance  = std::numeric_limits<double>::max() ; // the current minimum distance value
    ...
    {   size_t from_x = i ;
        for(size_t j=0; j<shift; j++)
        {
            // forward
            {   size_t from_y = j ;
                double d    = this->_distances[from_x][from_y][Constants::FORWARD] ;
                double norm = sum_distances - d ;
                this->_distances[from_x][from_y][Constants::FORWARD]  /= norm ;
                d           = this->_distances[from_x][from_y][Constants::FORWARD] ;
                if(d < min_distance)
                {   min_distance = d ;
            ...
            {   size_t j_rev = shift - j - 1 ;    // gives the complementary shift state -> if shift=3 : 0<->2, 1<->1, 2<->0
                from_y_rev = j_rev  ;             // the 1st element of the slice if considering <shift> assigned at the end
                double d    = this->_distances[from_x][from_y_rev][Constants::REVERSE] ;
                double norm = sum_distances - d ;
                this->_distances[from_x][from_y_rev][Constants::REVERSE] /= norm ;
                d           = this->_distances[from_x][from_y_rev][Constants::REVERSE] ;
                if(d < min_distance)
                {   min_distance = d ;
                    min_distances = std::vector<std::vector<size_t>>() ;
